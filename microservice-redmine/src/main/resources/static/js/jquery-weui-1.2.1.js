 ...
 jQuery WeUI V1.2.1 
 By 言川
 httplihongxun945.github.iojquery-weui
 
 global $true 
 global WebKitCSSMatrixtrue 
  ...
  $.Template7 = $.t7 = (function () {
    ...
    var Template7 = function (template) {
      ...
      function compile(template, depth) {
        ...
        var ctx = 'ctx_' + depth;
        var resultString = '(function (' + ctx + ', data) {n';
        if (depth === 1) {
          resultString += 'function isArray(arr){return Object.prototype.toString.apply(arr) === '[object Array]';}n';
          resultString += 'function isFunction(func){return (typeof func === 'function');}n';
          resultString += 'function c(val, ctx) {if (typeof val !== undefined) {if (isFunction(val)) {return val.call(ctx);} else return val;} else return ;}n';
        }
        resultString += 'var r = '';n';
        var i, j, context;
        for (i = 0; i  blocks.length; i++) {
          var block = blocks[i];
           Plain block
          if (block.type === 'plain') {
            resultString += 'r +='' + (block.content).replace(rg, 'r').replace(ng, 'n').replace('g, '' + ''') + '';';
            continue;
          }
...
function deprecate(method, name, message) {
    ...
    return function() {
        ...
        var stack = e && e.stack  e.stack.replace(^[^(]+[n$]gm, '')
            .replace(^s+ats+gm, '')
            .replace(^Object.anonymouss(gm, '{anonymous}()@')  'Unknown Stack Trace';
...
function splitStr(str) {
  ...
  @return {BooleanNumber} false when not found, or the index
...
function uniqueArray(src, key, sort) {
  ...
  @returns {StringUndefined} prefixed
...
var IE10_POINTER_TYPE_ENUM = {
    ...
    3 INPUT_TYPE_PEN,
    4 INPUT_TYPE_MOUSE,
    5 INPUT_TYPE_KINECT  see httpstwitter.comjacobrossistatus480596438489890816
};
...
inherit(SingleTouchInput, Input, {
  ...
  @returns {undefinedArray} [all, changed]
...
TouchAction.prototype = {
    ...
    preventDefaults function(input) {
        ...
        if (hasPanX && hasPanY) {
             `pan-x pan-y` means browser handles all scrollingpanning, do not prevent
            return;
        }
...
Recognizer.prototype = {
    ...
    set function(options) {
        assign(this.options, options);
         ...
         also update the touchAction, in case something changed about the directionsenabled state
    ...
    process function(inputData) { },  jshint ignoreline
...
function directionStr(direction) {
  ...
  @param {RecognizerString} otherRecognizer
...
inherit(SwipeRecognizer, AttrRecognizer, {
  ...
  A tap is ecognized when the pointer is doing a small tapclick. Multiple taps are recognized if they occur
...
Hammer.defaults = {
    ...
    domEvents false,
      ...
      The value for the touchAction propertyfallback.
    ...
    enable true,
      ...
      EXPERIMENTAL FEATURE -- can be removedchanged
      Change the parent input target element.
      If Null, then it is being set the to main element.
      @type {NullEventTarget}
      @default null
...
Manager.prototype = {
    ...
    recognize function(inputData) {
      ...
      @param {RecognizerString} recognizer
      @returns {RecognizerNull}
  ...
  addremove the css properties as defined in manager.options.cssProps
  @param {Manager} manager
  @param {Boolean} add
...
var freeGlobal = (typeof window !== 'undefined'  window  (typeof self !== 'undefined'  self  {}));  jshint ignoreline
...
+ function($) {
  ...
  $.modal = function(params, onOpen) {
    ...
    var tpl = 'div class=weui-dialog' +
                'div class=weui-dialog__hdstrong class=weui-dialog__title' + params.title + 'strongdiv' +
  ...
  $.prompt = function(text, title, onOK, onCancel, input) {
    ...
    var modal = $.modal({
      text 'p class=weui-prompt-text'+(config.text  '')+'pinput type=text class=weui-input weui-prompt-input id=weui-prompt-input value=' + (config.input  '') + ' ',
      title config.title,
      autoClose false,
      buttons [
      {
...
+ function($) {
  ...
  $.toast = function(text, style, callback) {
    ...
    show('i class=' + iconClassName + ' weui-icon_toastip class=weui-toast_content' + (text  已经完成) + 'p', className);
  ...
  $.showLoading = function(text) {
    ...
    html += 'i class=weui-loading weui-icon_toasti';
    html += 'div';
...
+ function($) {
  ...
  var show = function(params) {
    ...
    if (params.title) {
      titleHtml = 'div class=weui-actionsheet__titlep class=weui-actionsheet__title-text' + params.title + 'pdiv';
    }
    ...
    var tpl = 'div class=weui-actionsheet  id=weui-actionsheet'+
                ...
                'div class=weui-actionsheet__action'+
                  'div class=weui-actionsheet__cell weui-actionsheet_cancel取消div'+
                  'div'+
                'div';
    var dialog = $(tpl).appendTo(document.body);
...
+function ($) {
  use strict;
   ...
   fix httpsgithub.comlihongxun945jquery-weuiissues442
   chrome will always return 0, when use document.body.scrollTop
   httpsstackoverflow.comquestions43717316google-chrome-document-body-scrolltop-always-returns-0
...
;(function ($) {
    ...
    if (ipad  iphone  ipod) {
     ...
     iOS
    if (iphone && !ipod) {
        device.osVersion = iphone[2].replace(_g, '.');
        device.iphone = true;
    }
    if (ipad) {
        device.osVersion = ipad[2].replace(_g, '.');
        device.ipad = true;
    }
    if (ipod) {
        device.osVersion = ipod[3]  ipod[3].replace(_g, '.')  null;
        device.iphone = true;
    }
     iOS 8+ changed UA
...
+ function($) {
  ...
  var Picker = function (params) {
      ...
      p.columnHTML = function (col, onlyItems) {
          ...
          if (col.divider) {
              ...
              columnHTML += 'div class=picker-items-col ' + (col.textAlign  'picker-items-col-' + col.textAlign  '') + ' ' + (col.cssClass  '') + 'div class=picker-items-col-wrapper' + columnItemsHTML + 'divdiv';
          }
          ...
          pickerHTML =
              'div class=' + (pickerClass) + '' +
                  ...
                  'div class=picker-modal-inner picker-items' +
                      colsHTML +
                      'div class=picker-center-highlightdiv' +
                  'div' +
              'div';
      ...
      p.open = function () {
          ...
          if (!p.opened) {
              ...
              p.layout();
               ...
               Append
              if (toPopover) {
                  p.pickerHTML = 'div class=popover popover-picker-columnsdiv class=popover-inner' + p.pickerHTML + 'divdiv';
  ...
  $.fn.picker = function(params) {
    ...
    return this.each(function() {
      ...
      if(!picker) {
        params = $.extend({ input this }, params  {})  httpsgithub.comlihongxun945jquery-weuiissues432
...
+ function($) {
  ...
  Select.prototype._bind = function(dialog) {
    ...
    dialog.on(change, function(e) {
      var checked = dialog.find(inputchecked);
      ...
      'div class=weui-cells weui-cells_radio
        ...
        label class=weui-cell weui-check_label for=weui-select-id-{{this.title}}
          ...
          div class=weui-cell__ft
            ...
            span class=weui-icon-checkedspan
          div
        label
        {{items}}
      div',
  ...
  var Calendar = function (params) {
      ...
      function formatDate(date) {
          ...
          return p.params.dateFormat
              .replace(yyyyg, year)
              .replace(yyg, (year + '').substring(2))
              .replace(mmg, month1  10  '0' + month1  month1)
              .replace(mg, month1)
              .replace(MMg, p.params.monthNames[month])
              .replace(Mg, p.params.monthNamesShort[month])
              .replace(ddg, day  10  '0' + day  day)
              .replace(dg, day)
              .replace(DDg, p.params.dayNames[weekDay])
              .replace(Dg, p.params.dayNamesShort[weekDay]);
      }
      ...
      p.monthHTML = function (date, offset) {
          ...
          for (i = 1; i = rows; i++) {
              ...
              for (j = 1; j = cols; j++) {
                  ...
                  rowHTML += 'div data-year=' + dayYear + ' data-month=' + dayMonth + ' data-day=' + dayNumber + ' class=picker-calendar-day' + (addClass) + ' data-date=' + (dayYear + '-' + dayMonth + '-' + dayNumber) + 'span'+dayNumber+'spandiv';
              }
      ...
      p.onMonthChangeEnd = function (dir, rebuildBoth) {
          ...
          p.wrapper.find('.picker-calendar-monthnot(.picker-calendar-month-prev)not(.picker-calendar-month-current)not(.picker-calendar-month-next)').remove();
      ...
      p.layout = function () {
          ...
          var monthsHTML = 'div class=picker-calendar-monthsdiv class=picker-calendar-months-wrapper' + (prevMonthHTML + currentMonthHTML + nextMonthHTML) + 'divdiv';
           Week days header
          var weekHeaderHTML = '';
      ...
      p.open = function () {
          ...
          if (!p.opened) {
              ...
              p.layout();
               ...
               Append
              if (toPopover) {
                  p.pickerHTML = 'div class=popover popover-picker-calendardiv class=popover-inner' + p.pickerHTML + 'divdiv';
                  p.popover = $.popover(p.pickerHTML, p.params.input, true);
                  p.container = $(p.popover).find('.weui-picker-modal');
                  $(p.popover).on('close', function () {
                      onPickerClose();
  ...
  defaults = $.fn.calendar.prototype.defaults = {
    ...
    monthPickerTemplate 
        'div class=picker-calendar-month-picker' +
            'a href=javascript; class=link icon-only picker-calendar-prev-monthi class=icon icon-previa' +
            'div class=current-month-valuediv' +
            'a href=javascript; class=link icon-only picker-calendar-next-monthi class=icon icon-nextia' +
        'div',
    yearPicker true,
    yearPickerTemplate 
        'div class=picker-calendar-year-picker' +
            'a href=javascript; class=link icon-only picker-calendar-prev-yeari class=icon icon-previa' +
            'span class=current-year-valuespan' +
            'a href=javascript; class=link icon-only picker-calendar-next-yeari class=icon icon-nextia' +
        'div',
    weekHeader true,
     Common settings
    scrollToInput true,
...
+ function($) {
  ...
  defaults = $.fn.datetimePicker.prototype.defaults = {
      ...
      return [   自定义的时间
        {
          values (function () {
            var hours = [];
            for (var i=0; i24; i++) hours.push(formatNumber(i));
            return hours;
          })()
        },
        {
        ...
        },
        {
          values (function () {
            var minutes = [];
            for (var i=0; i60; i++) minutes.push(formatNumber(i));
            return minutes;
          })()
        }
      ];
    ...
    parse function (str) {
      ...
      return t[0].split(D).concat(t[1].split(时分秒)).filter(function (d) {
        return !!d;
      })
    }
  }
...
+function ($) {
  ...
  $.notification = $.noti = function(params) {
    ...
    if(!noti[0]) {  create a new notification
      noti = $('div class=weui-notificationdiv').appendTo(document.body);
      attachEvents(noti);
    }
  ...
  defaults = $.noti.prototype.defaults = {
    ...
    tpl  'div class=weui-notification__inner' +
            ...
            'div' +
            'div class=weui-notification__handle-bardiv' +
          'div'
  };
...
+ function($) {
  ...
  $.toptip = function(text, duration, type) {
    ...
    $t = $('div class=weui-toptipsdiv').appendTo(document.body);
    $t.html(text);
